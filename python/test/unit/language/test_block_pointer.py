...
def block_copy_kernel(
    ...
    PADDING_OPTION: tl.constexpr,
    TEST_LOWER_BOUND: tl.constexpr,
    TEST_UPPER_BOUND: tl.constexpr,
):
    pid = tl.program_id(0)
    ...
    a_block_ptr = tl.make_block_ptr(
        base=a_ptr, shape=(N // 2,), strides=(1,), offsets=(offset,), block_shape=(BLOCK_SIZE,), order=(0,)
    )
    b_block_ptr = tl.make_block_ptr(
        base=b_ptr, shape=(N,), strides=(1,), offsets=(offset,), block_shape=(BLOCK_SIZE,), order=(0,)
    )
...
@pytest.mark.parametrize(
    "dtypes_str, n, padding_option, boundary_check",
    [  #
...
def test_block_copy(dtypes_str, n, padding_option, boundary_check, device):
    ...
    block_copy_kernel[grid](
        a_ptr=a,
        b_ptr=b,
        N=n,
        BLOCK_SIZE=64,
        PADDING_OPTION=padding_option,
        TEST_LOWER_BOUND=boundary_check == "lower",
        TEST_UPPER_BOUND=boundary_check == "upper",
    )
    a.to(dst_dtype)
