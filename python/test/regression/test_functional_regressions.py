...
def test_chained_matmul(device):
    ...
    def chained_matmul_kernel(
        ...
        for loop_block_start in range(0, n, block_n):
            ...
            intermediate_mask = ((loop_block_start + tl.arange(0, block_n)) < n)[None, :] * (tl.arange(0, block_m) < m)[
                :, None
            ]
...
def test_vecmat(device):
    ...
    def batched_vecmat(
        ...
        output_tile = (m_index * block_m + tl.arange(0, block_m))[:, None] * dim_n + (
            n_index * block_n + tl.arange(0, block_n)
        )[None, :]
        ...
        for k_index in range(k_blocks):
            ...
            a_tile = (m_index * block_m + tl.arange(0, block_m))[:, None] * dim_k + (
                k_index * block_k + tl.arange(0, block_k)
            )[None, :]
            a = tl.load(A + a_tile)
            ...
            # leading dimension.
            b_tile = (
                (m_index * block_m + tl.arange(0, block_m))[None, :, None] * dim_n * dim_k
...
@pytest.mark.parametrize(
    "type", ["pre_load", "post_load", "post_pre_mixed", "post_load_two_iters", "post_load_three_iters"]
)
